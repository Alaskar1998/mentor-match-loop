...
const ChatMessageComponent = React.memo(({ 
  ...
  const isCurrentUser = useMemo(() => msg.senderId === currentUserId, [msg.senderId, currentUserId]);
  const isSystemMessage = useMemo(() => msg.type === 'system' || msg.message.startsWith('[SYSTEM]'), [msg.type, msg.message]);
...
const Chat = React.memo(() => {
  ...
  const currentUserName = useMemo(() => user?.name || user?.email || 'You', [user?.name, user?.email]);
  const currentUserAvatar = useMemo(() => user?.avatar_url, [user?.avatar_url]);
  const isCurrentUserInvolved = useMemo(() => 
    chatData && (chatData.user1_id === currentUserId || chatData.user2_id === currentUserId),
  ...
  const checkExchangeNotification = useCallback(async () => {
    ...
    try {
      ...
      if (contracts) {
        ...
        const userAgreed = isUser1 ? contracts.user1_agreed : contracts.user2_agreed;
        const otherUserAgreed = isUser1 ? contracts.user2_agreed : contracts.user1_agreed;
        const userFinished = isUser1 ? contracts.user1_finished : contracts.user2_finished;
        const otherUserFinished = isUser1 ? contracts.user2_finished : contracts.user1_finished;

        if (userAgreed && otherUserAgreed && !isExchangeActive) {
          setIsExchangeActive(true);
          setExchangeNotification('Exchange started! You can now begin your learning session.');
        } else if (userFinished && otherUserFinished && isExchangeActive) {
          setIsExchangeActive(false);
          setExchangeNotification('Exchange completed! You can now review your experience.');
  ...
  }, [newMessage, chatId, currentUserId, sending, otherUser?.id, currentUserName, createNotification]);
  ...
  const handleExchangeAgreed = useCallback(async (data: { userSkill: string }) => {
