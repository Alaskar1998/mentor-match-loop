-- Add function to manually insert learning responses for testing
CREATE OR REPLACE FUNCTION insert_test_learning_response(
...
-- Function to get responses for a user's learning requests with full details
CREATE OR REPLACE FUNCTION get_user_responses_with_details(p_user_id UUID)
RETURNS TABLE (
  response_id UUID,
...
GRANT EXECUTE ON FUNCTION insert_test_learning_response(UUID, UUID, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION get_user_responses_with_details(UUID) TO authenticated; 